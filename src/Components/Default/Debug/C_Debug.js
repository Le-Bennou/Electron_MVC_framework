...
const nodeIconBase64 = `data:image/svg+xml;base64,${btoa(`<?xml version="1.0" encoding="utf-8"?><!-- Uploaded to: SVG Repo, www.svgrepo.com, Generator: SVG Repo Mixer Tools -->
<svg width="32" height="32" viewBox="0 -183.5 512 512" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet">
...
export class Debug_Controller extends abstractController{

    onReady(){
        this.model.rerouteConsole()
    }
    ...
    setupEventListeners(){
     ...
     #formatConsoleMessage(type, data) {
  const { args, caller } = JSON.parse(data);

    
    const styles = {
        warn: 'color:rgb(214, 186, 103); font-weight: bold;',
        error: 'color:rgb(199, 98, 108); font-weight: bold;',
        log: 'color:rgb(97, 187, 118); font-weight: bold;'
    };
    ...
    const iconStyle = `
        ...
        background-image: url(${nodeIconBase64});
    ...
    `;
     ...
     return [
        `%c  %c${shortPath}:${caller ? caller.line : ''}\n%c`,
        iconStyle,
        locationStyle,
        styles[type],
        ...args
